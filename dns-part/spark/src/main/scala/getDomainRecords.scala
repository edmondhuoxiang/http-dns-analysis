...
object domainRecords extends Serializable {
	...
	def filter(sc: SparkContext, inFile: String, outDir: String) = {
		...
		val dir = inFile.split('/').apply(length-2)
		val file = inFile.split('/').apply(length-1)
		val file_1 = new File(outDir + dir)
        if(!file_1.exists){
        	file_1.mkdir()
        }
        val file_2 = new File(outDir + dir  +"/"+file)
        if(!file_2.exists)
            file_2.createNewFile()
        val fileWriter = new FileWriter(file_2.getAbsoluteFile(), true)
        val bufferwriter = new BufferedWriter(fileWriter)
